...
- `room` accepts a room id, alias (`#review` / `#review:server`), or the
  ...
  bound rooms, and anything not bound still fails `not_in_room`. A directory or
  room-name lookup failure now propagates instead of masquerading as
  `not_in_room`, and an ambiguous display name refuses rather than guessing.
